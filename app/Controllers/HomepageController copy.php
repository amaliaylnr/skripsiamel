...
use App\Models\SKeteranganModel;
...
class HomepageController extends BaseController
{
    ...
    function __construct()
    {
        ...
        $this->s_permohonan = new SPermohonanModel();
        $this->s_keterangan = new SKeteranganModel();
    ...
    {
        $data = [];
        return view('publik/tambah/form_permohonan', $data);
    }
    public function form_permohonan_proses()
    {
        $rules = [
            'nik'           => 'required|min_length[16]',
        ];
        $surat_permohonan = $this->s_permohonan;
        $warga = $this->warga;
        $imageFile = $this->request->getFile('identitas');
        if($this->validate($rules)){  
            $nama = $this->request->getVar('nama');
            ...
            if (empty($result)) {
                $gambarName = ($nik.'.'.$imageFile->getClientExtension());
                $imageFile->move(WRITEPATH . '../public/assets/surat/surat_permohonan/',$gambarName);
                $data_diri = [
                    'nik'       => $nik,
                    'nama'     => $nama,
                    'slug'     => $nik.'_'.$slugs,
                ...
                $surat_permohonan->saveNew($data_diri);
                $result2 = $warga->find($nik);
                ...
                $surat_permohonan->save($data_permohonan);
                return redirect()->to('pengajuan')->with('success', 'Data telah ditambahkan');
            ...
            elseif (!empty($result)) {
                ...
                if ($imageFile->isValid() && !$imageFile->hasMoved()) {
                    $old_img = $result['identitas'];
                    if (file_exists("../public/assets/surat/surat_permohonan/".$old_img)) {
                        unlink('../public/assets/surat/surat_permohonan/'.$old_img);
                    }
                    $gambarName = ($nik.'.'.$imageFile->getClientExtension());
                    $imageFile->move('../public/assets/surat/surat_permohonan/',$gambarName);
                    $data_diri['identitas'] = $gambarName;
            ...
            return redirect()->to('pengajuan/surat_permohonan')->with('success', 'Data tidak tersimpan');
    ...
    {
        $data = [];
        return view('publik/tambah/form_keterangan', $data);
    }
    public function form_keterangan_proses()
    {
        $rules = [
            'nik'           => 'required|min_length[16]',
        ];
        ...
        if($this->validate($rules)){  
            ...
            if (empty($result)) {
                $gambarName = ($nik.'.'.$imageFile->getClientExtension());
                $imageFile->move(WRITEPATH . '../public/assets/surat/surat_keterangan/',$gambarName);
                $data_diri = [
                    'nik'       => $nik,
                    'nama'     => $nama,
                    'slug'     => $nik.'_'.$slugs,
            ...
            elseif (!empty($result)) {
                ...
                if ($imageFile->isValid() && !$imageFile->hasMoved()) {
                    $old_img = $result['identitas'];
                    if (file_exists("../public/assets/surat/surat_keterangan/".$old_img)) {
                        unlink('../public/assets/surat/surat_keterangan/'.$old_img);
                    }
                    $gambarName = ($nik.'.'.$imageFile->getClientExtension());
                    $imageFile->move('../public/assets/surat/surat_keterangan/',$gambarName);
                    $data_diri['identitas'] = $gambarName;
            ...
            return redirect()->to('pengajuan/surat_keterangan')->with('success', 'Data tidak tersimpan');
    ...
    // surat kelahiran
    public function form_kelahiran()
    {
        $data = [];
        return view('publik/tambah/form_kelahiran', $data);
    }
    public function form_kelahiran_proses()
    {
        helper(['form']);
        $rules = [
            'nik'           => 'required|min_length[16]',
        ];
        $surat_kelahiran = $this->s_kelahiran;
        $warga = $this->warga;
        $imageFile = $this->request->getFile('identitas');
        if($this->validate($rules)){  
            $nama = $this->request->getVar('nama');
            ...
            if (empty($result)) {
                ...
                $data_kelahiran = [
                    ...
                    'keperluan' => 'Surat Keterangan Lahir Anak',
                    'pengambilan' => $this->request->getVar('pengambilan'),
                    'status' => 'proses',
                    'identitas' => $gambarName,
            ...
            elseif (!empty($result)) {
                ...
                $surat_kelahiran->save($data_kelahiran);
                return redirect()->to('pengajuan')->with('success', 'Data pengajuan <b>Surat Kelahiran </b>berhasil dikirim');
            }
            
            
            return redirect()->to('/pengajuan/surat_kelahiran')->with('success', 'Data tidak tersimpan');
